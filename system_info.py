...
def gpu_info(real_test: bool = False) -> dict[str, Any]:
    ...
    try:
        import torch
        info.update({'torch_version': torch.__version__, 'cuda_runtime': torch.version.cuda, 'arch_list': torch.cuda.get_arch_list() if torch.cuda.is_available() else []})
        if torch.cuda.is_available():
            free = total = None
            try: free, total = torch.cuda.mem_get_info(0)
            except Exception: pass
            props = torch.cuda.get_device_properties(0)
            info.update({'mode':'cuda','cuda_available':True,'gpu_name':torch.cuda.get_device_name(0),'compute_capability': f'{props.major}.{props.minor}', 'total_vram': total or props.total_memory, 'free_vram': free, 'used_vram': (total-free) if free is not None and total is not None else None, 'has_sm_120':'sm_120' in torch.cuda.get_arch_list()})
    except Exception as exc:
