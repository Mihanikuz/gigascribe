...
def is_gigaam_ready(base: Path, model_name: str) -> bool:
    ...
    gv,tv=_versions()
    return bool(m and m.get("model_name")==model_name and m.get("checkpoint_path",m.get("checkpoint"))==str(ckpt) and m.get("backend_version")==gv and m.get("torch_version")==tv and m.get("offline_verified") is True and ckpt.is_file() and ckpt.stat().st_size >= MIN_CHECKPOINT_BYTES and m.get("size",m.get("checkpoint_size"))==ckpt.stat().st_size and m.get("sha256",m.get("checkpoint_sha256"))==sha256_file(ckpt))
def write_gigaam_marker(base: Path, model_name: str, *, package_version: str="unknown", torch_version: str="unknown") -> None:
    ...
    atomic_write_json(gigaam_marker_path(base,model_name), {"model_id":meta["id"],"official_model_name":meta["official_model_name"],"model_name":model_name,"checkpoint_path":str(ckpt),"size":ckpt.stat().st_size,"sha256":sha256_file(ckpt),"backend_source":GIGAAM_SOURCE,"backend_version":package_version if package_version != "unknown" else gv,"backend_commit":GIGAAM_BACKEND_REF,"torch_version":torch_version if torch_version != "unknown" else tv,"device_used_for_test":"cpu","offline_verified":True,"created_at":datetime.now(timezone.utc).isoformat()})
...
def is_pyannote_ready(base: Path, model_id: str="pyannote-3.1") -> bool:
    ...
    return bool(meta and m and m.get("repo_id")==meta["repo_id"] and m.get("snapshot_path",m.get("path"))==str(target) and m.get("offline_verified") is True and (target/"config.yaml").is_file() and set(meta["nested_dependencies"]).issubset(set(m.get("nested_dependencies",m.get("nested_repos",[])))))
def write_pyannote_marker(base: Path, model_id: str="pyannote-3.1", *, nested_repos: list[str] | None=None) -> None:
    ...
    try:
        from pyannote import audio as pa; version=str(getattr(pa,"__version__","unknown"))
    except Exception: version="unavailable"
    atomic_write_json(pyannote_marker_path(base,model_id),{"model_id":model_id,"repo_id":SUPPORTED_DIARIZATION_MODELS[model_id]["repo_id"],"snapshot_path":str(target),"nested_dependencies":nested_repos if nested_repos is not None else SUPPORTED_DIARIZATION_MODELS[model_id]["nested_dependencies"],"pyannote_audio_version":version,"pipeline_result_format":"community-output" if model_id=="pyannote-community-1" else "annotation","offline_verified":True,"created_at":datetime.now(timezone.utc).isoformat()})
