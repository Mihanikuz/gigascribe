...
RUN --mount=type=cache,target=/root/.cache/pip pip install --upgrade pip && pip install -r ${TORCH_REQUIREMENTS}
COPY requirements-models.txt constraints.txt ./
RUN --mount=type=cache,target=/root/.cache/pip pip install -c constraints.txt -r requirements-models.txt
COPY requirements-base.txt requirements-gigaam.txt ./
RUN --mount=type=cache,target=/root/.cache/pip pip install -c constraints.txt -r requirements-base.txt && pip install --no-deps gigaam==0.1.0
COPY . .
